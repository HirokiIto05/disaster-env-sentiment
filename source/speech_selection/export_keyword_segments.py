...
def main():
    ...
    parser.add_option(
        "--hein-bound-dir",
        type=str,
        default="01_data/intermediate/text/hein-bound-tokenized",
        help="Hein bound input directory: default=%default",
    )
    parser.add_option(
        "--outfile",
        type=str,
        default="01_data/intermediate/text/selected_speech/keyword_segments_climate.jsonlist",
    ...
    files = sorted(glob(os.path.join(hein_bound_dir, "speeches_*.jsonlist")))
    files = [
        f
        for f in files
